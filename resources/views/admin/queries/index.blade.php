...
@section('title', 'All Queries - Nabha Learning')

@section('admin-content')
<div class="space-y-5">

    <div>
        <h1 class="text-2xl font-bold text-gray-800">{{ __('messages.queries.all_queries') }}</h1>
        <p class="text-gray-500 text-sm mt-1">{{ __('messages.queries.subtitle_admin') }}</p>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center">
            ...
            <p class="text-xs text-gray-500 mt-1">{{ __('messages.queries.pending') }}</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center">
            <p class="text-2xl font-bold text-emerald-600">{{ $replied }}</p>
            <p class="text-xs text-gray-500 mt-1">{{ __('messages.queries.replied') }}</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center md:col-span-1 col-span-2">
            <p class="text-2xl font-bold text-indigo-600">{{ $pending + $replied }}</p>
    ...
    <div class="flex gap-2">
        @foreach(['' => 'All', 'pending' => 'Pending', 'replied' => 'Replied'] as $val => $label)
            <a href="{{ route('admin.queries', ['status' => $val]) }}"
               class="px-4 py-2 text-sm font-medium rounded-xl transition
                      {{ $status === $val ? 'bg-indigo-600 text-white' : 'bg-white border border-gray-300 text-gray-600 hover:bg-gray-50' }}">
                {{ $label }}
            </a>
        @endforeach
    </div>
    ...
    <div class="space-y-4">
        @forelse($queries as $query)
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5" x-data="{ replyOpen: false }">
                        ...
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            @if($query->teacher)
                                <span class="text-xs bg-indigo-100 text-indigo-600 px-2 py-0.5 rounded-full">
                                    To: {{ $query->teacher->name }}
                                </span>
                            @else
                                <span class="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full">Open</span>
                            @endif
                ...
                @if($query->reply)
                    <div class="mt-3 border-l-4 border-emerald-400 bg-emerald-50 rounded-r-xl pl-4 pr-4 py-3 space-y-1">
                        <p class="text-xs font-semibold text-emerald-600">
                            Reply by {{ $query->repliedBy?->name ?? 'Admin' }}
                            @if($query->replied_at) · {{ $query->replied_at->format('d M Y, h:i A') }} @endif
                        </p>
                ...
                <div x-show="replyOpen" x-transition class="mt-3">
                    <form method="POST" action="{{ route('admin.queries.reply', $query) }}" class="space-y-3">
                        @csrf @method('PUT')
                        <textarea name="reply" rows="4" required
                                  placeholder="{{ __('messages.queries.reply_placeholder') }}"
                                  class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none">{{ $query->reply }}</textarea>
                        <div class="flex gap-2">
                            <button type="submit"
        ...
        <div>{{ $queries->withQueryString()->links() }}</div>
    </div>
</div>
@endsection
