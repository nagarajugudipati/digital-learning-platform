...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="text-gray-500 text-sm mt-1">Students enrolled in your courses or who attempted your quizzes</p>
    </div>
    ...
    <form method="GET" data-no-loading
            ...
            <div>
                ...
                <select name="course_id"
                        class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm
                               focus:outline-none focus:ring-2 focus:ring-emerald-400 bg-white transition">
        ...
        <div class="flex flex-wrap items-center gap-3 mt-4">
            <button type="submit"
                    class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-semibold transition">
                Apply Filters
            </button>
            @if($search || $courseId || $studentId || $dateFrom || $dateTo)
                ...
                <span class="text-xs text-gray-400">Filters active</span>
            @endif
        </div>
    </form>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        @php
            $cards = [
                ['label' => 'Students (this page)', 'value' => $report->count(),                                        'color' => 'text-gray-800',   'bg' => ''],
                ['label' => 'Lessons Completed',    'value' => $report->sum('completed_lessons'),                      'color' => 'text-emerald-600', 'bg' => ''],
                ['label' => 'Quiz Attempts',        'value' => $report->sum('quizzes_taken'),                          'color' => 'text-indigo-600',  'bg' => ''],
    ...
    @if($report->isEmpty())
        <div class="bg-white rounded-2xl py-20 text-center shadow-sm border border-gray-100">
            ...
            @if($search || $courseId || $studentId || $dateFrom || $dateTo)
                <a href="{{ route('teacher.reports') }}" class="mt-4 inline-block text-emerald-600 text-sm hover:underline">
                    Clear filters
                </a>
            @else
                <p class="text-gray-400 text-xs mt-2">Students appear here once they enroll in your courses or attempt your quizzes.</p>
            @endif
        </div>
    @else
                ...
                @foreach($report as $row)
                    <div class="p-4 space-y-2">
                        <div class="flex items-start justify-between gap-2">
                            ...
                            <span class="shrink-0 text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{{ $row['class_level'] }}</span>
                        </div>
                        <div class="grid grid-cols-3 gap-2 text-center text-xs pt-1">
                            <div class="bg-gray-50 rounded-lg p-2">
                                <div class="font-bold text-gray-700">{{ $row['completed_lessons'] }}/{{ $row['total_lessons'] }}</div>
                                <div class="text-gray-400 mt-0.5">Lessons</div>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-2">
                                <div class="font-bold text-gray-700">{{ $row['quizzes_taken'] }}</div>
                                <div class="text-gray-400 mt-0.5">Quizzes</div>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-2">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200 text-left">
                            ...
                            <th class="px-5 py-4 font-semibold text-gray-500 uppercase text-xs tracking-wide text-center">Lessons Done</th>
                            <th class="px-5 py-4 font-semibold text-gray-500 uppercase text-xs tracking-wide text-center">Total</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($report as $i => $row)
                            <tr class="hover:bg-emerald-50/30 transition-colors group">
                                ...
                                <td class="px-5 py-4">
                                    <div class="font-medium text-gray-800 group-hover:text-emerald-700 transition-colors">{{ $row['name'] }}</div>
                                ...
                                <td class="px-5 py-4 text-center text-gray-400">{{ $row['total_lessons'] }}</td>
